...
You can configure server settings for the `opencode serve` and `opencode web` commands through the `server` option. These settings are read from your **global config** at `~/.config/opencode/opencode.json` (CLI flags still override config).
